...
class CRM_addEmployee(unittest.TestCase,CRM_loggin):    #工作列表——添加员工
    ...
    def addEmployee_mudle(self,line_num):
        ...
        screenshot_path='./REPORT/'+'crm_addemployee'+now+'.png'
...
if __name__ == '__main__':
        sendlist=[]
        # 测试套件，构建测试集
        dis= unittest.defaultTestLoader.discover(r'crmTestSuite2/', pattern='runner0.py')

        # 我们要新建一个用于保存我们测试结果的文件，html
        now = time.strftime("%Y-%m-%d-%H-%M-%S")
        # 定义文件的名字
        filename = r'./REPORT/' + now + "_result.html"
        ...
        for i in os.listdir(r'crmTestSuite2/report/') :   # 遍历Report文件夹，将文件名保存在列表
            ...
            b=a.rstrip('.png')
            if b >=now:
                shotspath= r'./Report/'+i  #筛选大于程序开始时间的文件及截图
